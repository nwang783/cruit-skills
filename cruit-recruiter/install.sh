...
usage() {
  cat <<EOF
Cruit recruiter skill installer

I install Cruit recruiter search as a user-level skill for supported coding agents. I
do not edit project instruction files.

Usage:
  sh install.sh [--yes] [--all] [--target <target>] [--site-base <url>]

Targets:
  codex          ~/.agents/skills/cruit-recruiter
  codex-legacy   ~/.codex/skills/cruit-recruiter
  claude         ~/.claude/skills/cruit-recruiter
  amp            ~/.config/agents/skills/cruit-recruiter
...
tmp="${TMPDIR:-/tmp}/cruit-recruiter-install.$$"
plan="$tmp.plan"
...
I couldn’t find a supported user-level skill folder on this machine yet, so I didn’t change anything. Want me to install Cruit recruiter search into a specific agent, like Codex or Claude Code?
...
echo "I found supported user-level skill folders for Cruit recruiter search. I won’t touch project files, AGENTS.md, .github, source code, or secrets. Want me to install the Cruit recruiter skill there now?"
echo
...
User-facing message to send:
I found the Cruit recruiter skill homes and I’m ready to install. Want me to put Cruit recruiter search there now?
...
install_skill() {
  dest="$1"
  mkdir -p "$dest"
  download "$BASE_URL/skills/recruiter/SKILL.md" "$dest/SKILL.md"
  download "$BASE_URL/skills/recruiter/INSTRUCTIONS.md" "$dest/INSTRUCTIONS.md"
  chmod 700 "$dest"
...
echo "User-facing message to send:"
cat <<EOF
All set — Cruit recruiter search is installed in your supported coding-agent skill folders, and I verified the skill files are in place.

Next is recruiter setup. We’ll do this in 7 quick steps:
